...
const useContract = () => {
  ...
  const writeContract = (write: {
    ...
    const { method, abi, contract, provider, config } = write;
  ...
  const readContract = (read: {
    ...
    const { method, abi, contract, provider, config, format, decimal } = read;
    if (!method || !abi || !contract || !provider || !config) {
      throw new Error('合约参数不全');
    }
    return new Promise(async (resolve, reject) => {
  ...
  const getTokenAmount = async (data: {
    ...
    const { provider, abi, contract, decimal, account } = data;
